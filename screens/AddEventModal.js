  ...
  const handleNext = () => {
    if (step === 1) {
      ...
      if (selectedDays.length === 0 || !startTime || !endTime) {
        Alert.alert('Missing Info', 'Please select at least one day and time.');
        return;
      }
      setStep(3);
    }
      ...
      <View style={styles.overlay}>
        <View style={[
          ...
          <View contentContainerStyle={styles.innerModal}>
            ...
            {step === 2 && (
              <>
                <Text style={styles.header}>Choose Days & Time</Text>
                <View style={styles.scrollPickerContainer}>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.daySelector}>
                    {DAYS.map(day => (
                      <TouchableOpacity
                        key={day}
                        style={[styles.dayOption, selectedDays.includes(day) ? styles.selectedDay : null]}
                        onPress={() => toggleDaySelection(day)}
                      >
                        <Text style={selectedDays.includes(day) ? styles.selectedDayText : styles.dayOptionText}>
                          {day}
                        </Text>
                      </TouchableOpacity>
                    ))}
            ...
            {step === 3 && (
              <>
                ...
                <ScrollView contentContainerStyle={styles.reviewScrollContainer}>
                  ...
                  <Text style={styles.summaryText}>
                    <Text style={{ fontWeight: 'bold' }}>Days:</Text> {selectedDays.join(', ')}
                  </Text>
                  <Text style={styles.summaryText}>
...
const styles = StyleSheet.create({
  ...
  dateAndTimeModalContent: {
    backgroundColor: 'white', borderRadius: 20, padding: 25, width: '95%', height: '52%',
  },
  reviewModalContent: {
    backgroundColor: 'white', borderRadius: 20, padding: 25, width: '85%', height: '40%',
  },
  innerModal: {
